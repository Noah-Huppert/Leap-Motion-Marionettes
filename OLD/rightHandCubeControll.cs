...
public class rightHandCubeControll : MonoBehaviour {
	...
	void Update () {
		...
		if(controller.IsConnected) //controller is a Controller object
        {
			...
			if(frame.Hands.Count > 0){
				lastKnownHandPos = new Vector3(((rightHandPos.x / 50) * -1), rightHandPos.y / 50, rightHandPos.z / 50);
			}
			
			if(frame.Hands.Count == 0){
				transform.localPosition = lastKnownHandPos;
